...
树的话，无非就是前中后序遍历、二叉树、二叉搜索树、平衡二叉树，更高级一点的有红黑树、B树、B+树，以及字典树。

二叉排序树就是左边结点比根节点小，右边比根节点大，并且左右子树都是二叉排序树，在一些极端情况，比如插入序列是有序的，就会出现退化的情况。二叉排序树退化成链表，所以才需要平衡树（AVL），在插入的同时调整这棵树，让它的结点尽可能均匀分布。而红黑树其实就是红黑树的一种，它定义和规则都比较复杂，但其实他最终的目的都是为了保证树的平衡性。因为树的查找性能就取决于树的高度，让树尽可能平衡，就是为了降低数的高度。B树是一种多路搜索树，它的每个节点可以拥有多于两个孩子节点。M路的B树最多能拥有M个孩子节点，至于为什么要设计成多路的，就是为了能进一步降低树的高度，路数越多，输的高度越低，但如果设置成无限多路，就会退化成有序数组了。而文件系统的索引就喜欢用B树而不用红黑树或者有序数组，因为文件系统和数据库的索引都是存在磁盘上的，并且如果数据量大的话，不一定能一次性加载到内存中，如果一棵树都无法一次性加载进内存，那么查找就无从谈起了。此时B树的多路存储的作用就显现出来了，可以每次加载B树的一个结点，然后一步步往下找。在内存中，红黑树比B树效率更高（红黑树更多的计算），但是涉及到磁盘操作，B树就更有优势了。

​	B+树是B树基础上进行改造，它的数据都在叶子结点，同时叶子结点之间还加了指针形成链表。B+树在数据库的索引中用得比较多，这是和业务场景相关的，比如从数据库中select数据，不一定只选一条，很多时候是批量查询的。如果是多条的话，B树需要做局部的终须遍历，可能要跨层访问，而B+树由于所有数据都在叶子结点，不需要跨层，同时由于有链表结构，只需要找到收尾，通过链表就能把所有数据取出来了。相同的道理，虽然hash比B+树更快，但hash也不适合用来查询大批数据。
...
- ![image-20201113170532038](C:\Users\吴宝\AppData\Roaming\Typora\typora-user-images\image-20201113170532038.png)
