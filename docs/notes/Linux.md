...
# 常用命令
- 磁盘存储
df  显示指定磁盘文件的可用空间。  df [选项] [文件]

du 显示每个文件和目录的磁盘使用空间。 du [选项] [文件]

- 性能监控和优化 
top 显示当前系统正在执行的进程的相关信息，包括进程ID、内存占用率、CPU占用率等。  top [参数]

free 显示系统使用和空闲的内存情况，包括物理内存、交互区内存(swap)和内核缓冲区内存。 free [参数]

vmstat 用来显示虚拟内存的信息。  

iostat  查看CPU、网卡、tty设备、磁盘、CD-ROM 等等设备的活动情况, 负载信息。 iostat [参数] [时间] [次数]

lsof  用于查看你进程开打的文件，打开文件的进程，进程打开的端口(TCP、UDP)。 lsof [参数] [文件]

- 网络
ifconfig 命令用来查看和配置网络设备。   ifconfig [网络设备] [参数]

route 命令是用于操作基于内核ip路由表，它的主要作用是创建一个静态路由让指定一个主机或者一个网络通过一个网络接口，如eth0。

ping 确定网络和各外部主机的状态；跟踪和隔离硬件和软件问题；测试、评估和管理网络。 ping [参数] [主机名或IP地址]

traceroute 追踪网络数据包的路由途径，预设数据包大小是40Bytes，用户可另行设置。  traceroute [参数] [主机]

netstat 用于显示与IP、TCP、UDP和ICMP协议相关的统计数据，一般用于检验本机各端口的网络连接情况。

telnet 执行telnet指令开启终端机阶段作业，并登入远端主机。 telnet [参数] [主机]
