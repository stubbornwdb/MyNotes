...
`static` 变量在 Java 中是属于类的，它在所有的实例中的值是一样的。当类被 **Java 虚拟机载入**的时候，会对 `static` 变量进行初始化。
...
- 首先，在 String 池内找，如果找到 `"xyz"` 字符串，则不创建 `"xyz"` 对应的 String 对象，否则创建一个对象。
...
[见此文](https://juejin.im/post/6844903806577164302)
